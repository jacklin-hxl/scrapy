...
class ZhihuQuestionItem(scrapy.Item):
    ...
    comments_num = scrapy.Field(input_processor=MapCompose(handle_data.extract_num,handle_data.handle_num))
    answer_num = scrapy.Field(input_processor=MapCompose(handle_data.handle_num),)
    attention_num = scrapy.Field(input_processor=MapCompose(handle_data.handle_num),)
    click_num = scrapy.Field(input_processor=MapCompose(handle_data.handle_num),)
    ...
    def save_to_es(self):
        ...
        article.suggest = es_method.gen_suggests(article._doc_type.index,((article.title,10),(article.topics,7),(article.content,4)))
        article.save()
...
class ZhihuAnswerItem(scrapy.Item):
    ...
    def get_insert_sql(self):
        ...
        """
        params = []
        params.append(self.get("question_title",""))
