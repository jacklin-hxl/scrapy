...
import scrapy
from ScrapyRedisTest.items import t1Item,t1ItemLoader
from utils.common import get_md5
from scrapy.loader import ItemLoader
from scrapy_redis.spiders import RedisSpider

class T1Spider(RedisSpider):
    name = 't1'
    allowed_domains = ['news.cnblogs.com']
    # start_urls = ['https://news.cnblogs.com/']
    redis_key = 't1:start_urls'
    ...
    def parse_detail(self,response):
        ...
        if match_re:
            ...
            job_item = t1.load_item()
            yield scrapy.Request(url=parse.urljoin(response.url,'/NewsAjax/GetAjaxNewsInfo?contentId={}'.format(post_id)),meta={"job_item":job_item},callback=self.parse_nums)
    ...
    def parse_nums(self,response):
        ...
        t1_job_item = t1ItemLoader(item=job_item,response=response)
        t1_job_item.add_value("diggcount",j_data["DiggCount"])
        t1_job_item.add_value("totalview",j_data["TotalView"])
        t1_job_item.add_value("commentcount",j_data["CommentCount"])
        t1_job_item.add_value("burycount",j_data["BuryCount"])
        ...
        # t1_job_item = t1.load_item()
        _t1_job_item = t1_job_item.load_item()
        return _t1_job_item
